...
pub(crate) struct SessionManagerEventStream<'a> {
    ...
    session_event_receiver: Receiver<SessionManagerEvent>,
}

impl<'a> SessionManagerEventStream<'a> {
    pub(crate) fn new(session_manager: &'a SessionManager<'a>) -> Result<Self, Error> {
        let (sender, receiver) = channel();

        let session_manager_event_stream = Self {
            session_manager,
            session_manager_event_client: SessionManagerEventClient::new(sender).into(),
...
impl<'a> Deref for SessionManagerEventStream<'a> {
    type Target = Receiver<SessionManagerEvent>;
